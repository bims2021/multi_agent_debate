...
A sophisticated LangGraph-based system that simulates structured debates between AI agents with different professional personas. Features dynamic agent selection, memory management, and automated judgment.

## Features

- **Dynamic Agent System**: Choose from multiple professional personas (Scientist, Philosopher)
...
- **Comprehensive Logging**: Full state tracking and debate analytics
- **Visualization**: Automatic graph diagram generation
...
Extensible: Easy to add new agent types and debate rules
...
- **State Management**: TypedDict with debate phases and agent memories
- **Base Classes**: Extensible foundations for nodes and agents
- **Registry Pattern**: Dynamic agent creation and management
...
Select agents (or press Enter for defaults)
...
└─────────────────────────────────────────────────────────────┘
    ...
    └──────┬───────┘
           ...
           ▼
    ┌──────────────┐
    │ Round Ctrl   │  ← Check completion
    └──────┬───────┘
           │
           ▼
    ┌──────────────┐
    │   Agent B    │  ← Generate response
    └──────┬───────┘
           │
           ▼
    ┌──────────────┐
...
class HistorianAgent(LLMAgent):
    def __init__(self, agent_id: str = "historian", config: dict = None):
        default_config = {
            ...
            'system_prompt': """You are a historian who analyzes
            current issues through historical patterns..."""
        }
...
def is_valid_argument(self, argument: str, used_arguments: List[str]) -> bool:
    return (
        ...
        self.custom_rule(argument)  # Add your rule
    )
